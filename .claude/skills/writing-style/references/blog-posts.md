...
- `[extra] series` + `series_order`: optional, when the post belongs to a series (keys in `config.toml` under `[extra.series]`).
- `related_posts` (usually 3) and `related_readings` (usually 2-4): repo-relative paths. Preferred over a trailing `## Related` section.

## Body

- Short intro hook, then `<!-- more -->` before the first `##`.
- **5 to 7 H2 sections.** Never skip levels. H3 only to enumerate named sub-parts under one H2 (e.g. "### Level 0", "### Level 1").
...
- Most H2 sections close on a `>` punchline.
...
5. 5 to 7 H2s, no skipped levels, every section earns its place.
