...
(function () {
  ...
  // The payload is built by templates/profile.html. A single unquoted value in
  // there makes the whole block invalid JSON, and then this page silently renders
  // an empty dashboard in English, so normalise the parse result rather than
  // trusting the shape: only the catch used to supply the defaults, and a payload
  // that parsed to null or lacked `series` still threw further down.
  var parsed = null;
  ...
  var t = es ? {
    ...
    favorites: 'Guardados',
    favoritesHint: 'Los posts que has guardado para volver a ellos.',
    empty: 'Aún no has terminado ningún post. Cuando leas uno hasta el final, aparecerá aquí.',
    emptyFavs: 'Aún no has guardado ningún post. Usa el botón de marcador en la cabecera de un post para guardarlo.',
    ...
    favorites: 'Saved',
    favoritesHint: 'Posts you saved to come back to.',
    empty: "You haven't finished any post yet. When you read one to the end, it'll show up here.",
    emptyFavs: "You haven't saved any posts yet. Use the bookmark button at the top of a post to save it.",
    reset: 'Clear my data',
  ...
  function loadJSON(key) {
    try { return asTimestampMap(JSON.parse(localStorage.getItem(key))); }
    catch (e) { return {}; }
  }
