+++
title = "Forming, Storming, Norming y Performing"
description = "Para que un equipo alcance alto rendimiento, hay que entender el Modelo de Tuckman: forming, storming, norming, performing y adjourning. Aquí exploro estrategias prácticas para cada etapa."
draft = false
[taxonomies]
tags = ["leadership", "team-management", "psychology", "communication"]
[extra]
subtitle = "El Modelo de Tuckman para llevar a un equipo a alto rendimiento"
static_thumbnail = "/images/blog/2023-11-25/cover.jpg"
related_posts = [
  "blog/2022-08-22-understanding-people.md",
  "blog/2023-02-27-great-leadership.md",
]
related_readings = [
  "readings/2021-09-12-turn-the-ship-around.md",
  "readings/2021-12-07-the-five-dysfunctions-of-a-team.md",
  "readings/2022-09-30-dare-to-lead.md",
]
+++

![blog-cover](/images/blog/2023-11-25/cover.jpg)

En 1965, el psicólogo Bruce Tuckman creó un modelo que describe cómo un grupo se forma y madura hasta convertirse en un equipo cohesivo y efectivo.

<!-- more -->

El modelo inicialmente consistía en cuatro etapas: "_**forming**, **storming**, **norming**,_ y _**performing**_," añadiendo una adicional "_**adjourning**_" en 1977.

Los equipos no siempre avanzan de forma lineal por estas etapas. A veces vuelven a una etapa anterior según las circunstancias.

## Forming

En esta etapa inicial, los miembros del equipo son cordiales pero tímidos, inseguros sobre sus roles. Dependen del líder para orientarse.

> **Objetivo**: Los miembros se están conociendo, y hay un enfoque en definir el propósito, objetivos y roles del equipo.

### Enfoque de liderazgo

- Proporcionar dirección y orientación claras
- Definir claramente los objetivos, roles y expectativas del equipo
- Actuar como facilitador, fomentando la comunicación abierta y ayudando a los miembros del equipo a conocerse.

## Storming

Surgen conflictos y desacuerdos cuando los miembros empiezan a expresar su individualidad. Pueden aparecer luchas de poder y desafios a la autoridad del lider.

> **Objetivo**: El equipo clarifica sus objetivos, los miembros aprenden a resolver conflictos y abordar diferencias de manera constructiva.

### Enfoque de liderazgo

- Facilitar la resolución de conflictos
- Reconocer y abordar conflictos de manera constructiva
- Fomentar la comunicación abierta y honesta mientras guía al equipo a través del proceso de entender y apreciar perspectivas diversas

![cover](/images/blog/2023-11-25/middle.jpg)

## Norming

La cohesion empieza a desarrollarse. Los miembros establecen normas y valores. Los roles se aclaran y surge un sentido de unidad.

> **Objetivo**: El equipo se esfuerza por establecer normas, valores y un entendimiento compartido. Los miembros aprenden a apreciar las fortalezas y debilidades de cada uno.

### Enfoque de liderazgo

- Fomentar la colaboración y la inclusividad
- Animar a los miembros del equipo a establecer normas y valores colectivamente
- Reconocer y celebrar las fortalezas individuales, fomentando un sentido de unidad y respeto mutuo

## Performing

El equipo funciona a pleno rendimiento, enfocado en lograr sus objetivos. Los miembros colaboran, confian entre si y se apoyan mutuamente.

> **Objetivo**: El equipo está comprometido con su propósito común y opera a un alto nivel de eficiencia y efectividad.

### Enfoque de liderazgo

- Empoderar la autonomía y la confianza
- Proporcionar oportunidades para que los miembros del equipo tomen propiedad de tareas y proyectos
- Fomentar un ambiente donde los individuos se sientan seguros de sus habilidades y puedan colaborar sin problemas

## Adjourning (o Mourning)

Esta etapa marca el final de la tarea o proyecto. Los miembros pueden sentir cierta perdida cuando el grupo se disuelve.

> **Objetivo**: Reconocer y celebrar los logros del equipo, proporcionar cierre y reflexionar sobre la experiencia general.

### Enfoque de liderazgo

- Reconocer logros y proporcionar cierre
- Reconocer los logros del equipo y expresar gratitud por las contribuciones individuales
- Facilitar una sesión reflexiva para capturar lecciones aprendidas y crear una experiencia de cierre positiva


![cover](/images/blog/2023-11-25/footer.jpg)

Para llevar un equipo a alto rendimiento usando el [Modelo de Tuckman](https://en.wikipedia.org/wiki/Tuckman's_stages_of_group_development), los **lideres** deben conocer las etapas y **adaptar** su estilo segun la situacion.

Esto significa dar **orientacion y estructura** en _forming_, **facilitar la resolucion de conflictos** en _storming_, **fomentar colaboracion y comunicacion** en _norming_, **dar autonomia** en _performing_, y **reconocer logros** en _adjourning_.

> La comunicacion regular, las actividades de team-building y resolver conflictos de forma constructiva son claves durante todo el proceso.

---

### Resumen en video

{{ youtube(id="-RwkZxGPQb8") }}
