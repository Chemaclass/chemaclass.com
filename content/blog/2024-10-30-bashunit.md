+++
title = "bashunit"
description = "bashunit: a testing framework for Bash with parallel and snapshot testing, test doubles and data providers. From dev frustration to open source."
draft = false
[taxonomies]
tags = [ "bashunit", "developer-tools", "testing", "open-source", "tdd" ]
[extra]
subtitle = "Turning frustrations into tools for better development"
static_thumbnail = "/images/blog/2024-10-30/cover.webp"
related_posts = [
...
bashunit is a lightweight, easy-to-use [testing](/blog/the-art-of-testing/) framework for Bash. Features like parallel and snapshot testing, test doubles, data providers, and tons of built-in assertions. 
...
![bashunit-original-assert.jpg](/images/blog/2024-10-30/bashunit-original-assert.webp)

The `assert` function allowed me to define multiple assertions in a separate file, making it easy to validate that any [refactoring](/blog/the-art-of-refactoring/) of the original hook maintained the expected behavior. If a change inadvertently broke existing functionality, it would instantly flag the issue, letting me know right away that something needed fixing. This setup provided immediate [feedback](/blog/ship-show-ask/) and helped ensure that any updates to the script didn't disrupt its intended logic. For example:

![conventional-commits-original-tests.jpg](/images/blog/2024-10-30/conventional-commits-original-tests.webp)
...
![conventional-commits-call_test_functions.jpg](/images/blog/2024-10-30/conventional-commits-call_test_functions.webp)
...
![conventional-commits-refactor-test-cases.jpg](/images/blog/2024-10-30/conventional-commits-refactor-test-cases.webp)
...
![bashunit idea note](/images/blog/2024-10-30/bashunit-idea.webp)
...
On September 4, 2023, I launched version  [0.1](https://github.com/TypedDevs/bashunit/commit/fc9aac40eb8e5ad4483f08d79eb678a3650dcf78), which featured a working runner and a single assertion function: `assertEquals`. Later, version [0.2](https://github.com/TypedDevs/bashunit/releases/tag/0.2.0) was released, allowing `./bashunit` to be a standalone executable, runnable from any folder. Here's how it looked back then:

![bashunit-02-demo.jpg](/images/blog/2024-10-30/bashunit-02-demo.webp)
...
![bashunit-ci.jpg](/images/blog/2024-10-30/bashunit-ci.webp)

In June 2024, bashunit was [integrated in PHPStan](https://bashunit.com/blog/2024-06-21-phpstan-integration)  for their end-to-end tests, enabling the use of bashunit's assertions independently of its runner. This flexibility turned out to be very useful.
...
It also supports a wide range of [command-line parameters](https://bashunit.com/command-line)
and [configuration values](https://bashunit.com/configuration). Some of my favorites include:
...
We also provide [data providers](https://bashunit.com/data-providers) for running the same test cases with different inputs.

For [test doubles](https://bashunit.com/test-doubles), bashunit offers mocks and spies. These work within the same process as the test, but currently, they don't work across processes, an area for improvement.

Powerful [snapshot testing](https://bashunit.com/snapshots) is included, making it easy to verify command or script outputs over time.

bashunit offers a large set of native [assertions](https://bashunit.com/assertions) for test cases, including:
...
You can even create your own [custom asserts](https://bashunit.com/custom-asserts) to extend bashunit's capabilities.
