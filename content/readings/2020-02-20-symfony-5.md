+++
title = "Symfony 5"
description = "Fabien Potencier's official guide to building web applications with Symfony 5, covering the framework from scratch to production deployment."
authors = [ "Fabien Potencier" ]
[taxonomies]
tags = [ "php", "architecture" ]
[extra]
subtitle = "Symfony 5: The Fast Track"
pages = "320"
author = "Fabien Potencier"
static_thumbnail = "/images/readings/symfony-5.webp"
related_readings = [
  "readings/2020-08-16-advance-web-application-architecture.md",
  "readings/2016-10-01-the-pragmatic-programmer.md",
  "readings/2018-06-04-clean-architecture.md",
]
+++

<!-- more -->

This book, written by Symfony's creator, lays out a [pragmatic](/readings/the-pragmatic-programmer/) approach to developing [web applications](/readings/advance-web-application-architecture/) with Symfony 5:
from scratch to production. Whether you are discovering Symfony for the first time or refreshing your knowledge, this
practical guide provides the definitive introduction to modern Symfony applications.
