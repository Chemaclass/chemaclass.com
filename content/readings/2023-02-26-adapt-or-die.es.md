+++
title = "Adapt or die"
description = "A través de la historia de People First IT, el libro presenta El Algoritmo del Éxito: transformar la organización desde las personas."
draft = false
authors = [ "Thomas H. Douglas" ]
[taxonomies]
tags = [ "leadership", "team-management", "communication", "productivity", "philosophy" ]
[extra]
subtitle = "Cómo Crear Innovación, Resolver Puzzles de Personas y Ganar en los Negocios"
pages = "270"
author = "Thomas H. Douglas"
static_thumbnail = "/images/readings/adapt-or-die.webp"
related_readings = [
  "readings/2021-01-16-who-moved-my-cheese.md",
  "readings/2023-01-29-the-infinite-game.md",
  "readings/2019-11-12-atomic-habits.md",
]
+++

A través de la historia de People First IT, este libro presenta El Algoritmo del Éxito: un sistema con potencial para transformar todos los aspectos de una organización.

<!-- more -->

Cada capítulo revela parte de la fórmula que las pequeñas y medianas empresas necesitan para triunfar. Con ejemplos reales, investigación y herramientas prácticas, el libro explica cómo crear innovación, resolver problemas de personas y ganar en los negocios.

> "El [liderazgo](/es/blog/the-beauty-of-leadership/) consiste en ayudar a las personas a adaptarse al cambio para que el negocio y su gente prosperen."

## ¿Por qué fracasan los negocios?

Los negocios fracasan porque...

- no pueden o no quieren tener conversaciones valientes
- se enfocan en el dinero demasiado pronto, olvidando a las personas
- saben crear valor pero no escalarlo
- no priorizan a las personas como responsabilidad principal
- se enfocan en las personas pero carecen de las habilidades para marcar diferencia
- escuchan para responder, no para entender
- creen que sus problemas son únicos porque su idea lo es
- viven dentro del negocio sin trabajar sobre el negocio
- esperan que las cosas pasen sin liderar el cambio
- luchan contra la verdad en lugar de abrazarla

## ¿Cómo hacer crecer tu equipo?

> "Cuando un negocio escala, el foco del dueño y el liderazgo debe pasar de los clientes a los empleados."

La clave está en tener conversaciones valientes sobre las habilidades necesarias para el siguiente nivel. El objetivo: mejorar la experiencia profesional de cada persona y de quienes la rodean.

Puntos clave que todos podrían desarrollar:

- Buenas comunicaciones
- Enfoque y lograr metas y resultados
- Todos contribuyen
- Ofrecerse apoyo mutuamente
- Buen liderazgo
- Organización clara y buena
- El [conflicto](/es/readings/the-five-dysfunctions-of-a-team/) constructivo impulsa la innovación

> "Las personas no suelen ser el problema raíz. La taxonomía es 3Ps: producto, proceso o personas. 'Personas' va al final porque los líderes deben revisar primero producto y proceso. Es decir: las personas primero en valores, pero al último en culpa."

## El Algoritmo del Éxito

Como muestra el diagrama, todo esto depende del liderazgo. Si liderar es ayudar a otros a adaptarse al cambio, debe ser central en nuestras organizaciones.

![El Algoritmo del Éxito dibujado como un templo, con el liderazgo apoyado en siete pilares: producto, estrategia, motor de crecimiento, modelo, personas, operaciones y procesos](/images/readings/2023-02-26/aos-leadership.webp)

### Producto

- Promesas
- Resolver un problema o necesidad
- Disparadores de liberación de efectivo
- La forma en que creamos valor
- Dolor o Placer
- Innovación
- Conexiones: Lógicas, emocionales y competitivas

### Estrategia

- Perfil del cliente objetivo (Quién)
- Estrategia de Creación de Valor (VCS)
- Valores Fundamentales
- Visión compartida
- Ciclo de valor
- Procesos clave

### El Motor de Crecimiento

- Propuesta única de venta
- Marketing dirigido
- Canales de venta
- Pasos de venta
- Medición del Motor de Crecimiento
- Gestión de embudo y oportunidades
- Retorno sobre Ventas

### El Modelo Financiero

- Gestión de ingresos
- Costo de bienes vendidos (COGS)
- Costos de ventas y marketing
- Administración general
- EBITDA, ITDA, NOI
- Categorización
- Reportes

### Personas

- Alineación
- Elevadores de personas
- Resolución de problemas
- Bancos de personas
- Código de conducta del liderazgo
- Planes de carrera
- One-on-one's
- Gestión de ingresos

### Operaciones

- Identificar las operaciones centrales
- Responsabilidades
- Gestión de cadencia
- Reuniones
- GSD (Get Shit Done)

### Proceso

- Promesas Cumplidas
- Downstream / Upstream
- Responsabilidad
- Repetible
- Eficiencias

---

Puedes encontrar el libro en [https://www.adaptordie.com/the-book/](https://www.adaptordie.com/the-book/).
